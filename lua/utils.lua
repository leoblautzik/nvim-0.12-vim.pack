...
M.open_floating = function()
  ...
  local grp = vim.api.nvim_create_augroup("FloatTerm_" .. buf, { clear = true })

  -- Cerrar automáticamente al salir del proceso
...
M.open_small = function()
  ...
  -- Cerrar la ventana automáticamente al terminar el proceso
  vim.api.nvim_create_autocmd("TermClose", {
    group = grp,
    buffer = term_buf,
    callback = function()
...
local function run_cmd_output(cmd, cwd)
  ...
  local grp = vim.api.nvim_create_augroup("Runner_" .. buf, { clear = true })
...
function M.run_file()
  ...
  if ft == "lua" then
    run_cmd_output({ "lua", file }, dir)
  elseif ft == "c" then
    local out = "/tmp/a.out"
    local compile = { "gcc", file, "-o", out }
  ...
  elseif ft == "go" then
    ...
    local root = gomod ~= "" and vim.fn.fnamemodify(gomod, ":h") or dir

    if file:match("_test%.go$") then
      run_cmd_output({ "go", "test" }, dir)
    else
      run_cmd_output({ "go", "run", file }, root)
    end
  elseif ft == "java" then
    local out = dir .. "/bin"
    ...
    vim.fn.mkdir(out, "p")
    local result = vim.fn.system({ "javac", "-d", out, file })

    if vim.v.shell_error ~= 0 then
      print("Error de compilación:\n" .. result)
    else
      run_cmd_output({ "java", "-cp", out, class }, dir)
    end
  else
